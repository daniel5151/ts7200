...
// TODO: burn this to the ground and implement a proper gdb stub
...
impl Asm2Line {
    ...
    pub fn load_objdump(&mut self, objdump_fname: &str) -> std::io::Result<()> {
            ...
            match state {
                ...
                ParseState::Disasm {
                    ...
                    if ln.ends_with("():") {
                        // new function name
                        *cur_file_ln = None
                    } else if ln.starts_with('/') || ln.starts_with('.') || ln.starts_with('_') {
                        // file + line number
        ...
        for (fname, content) in self.file_content.iter_mut() {
            let file = BufReader::new(match File::open(&fname) {
                Ok(f) => f,
                Err(_) => continue,
            });
